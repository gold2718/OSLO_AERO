module oslo_aero_ndrop
  ...
  real(r8), parameter :: eps = 1.e-30_r8     ! Avoid divide by zero
  real(r8), parameter :: min_hygro = 0.01_r8 ! XXgoldyXX: why?
...
!===============================================================================
  ...
  subroutine ndrop_init_oslo()
    ...
    do m=1,ntot_amode
       ...
       if(m < 10) modeString="0"//adjustl(modeString)
  ...
  subroutine dropmixnuc_oslo( state, ptend, dtmicro, pbuf, wsub,                    &
     ...
     real(r8)              :: sumFraction
     logical               :: num_neg ! For trapping negative number conc.
     ...
     do n=1,nmodes
        ...
        if(n < 10) modeString="0"//adjustl(modeString)
        varName = "NMR"//trim(modeString)
     ...
     do k = top_lev, pver
        mm = k - top_lev + 1
        do m = 1, nmodes
           if(ANY(numberConcentration(:ncol,k,m) < 0.0_r8 )) then
              num_neg = num_neg + 1
              tmpa = minval(numberConcentration(:ncol,k,m))*1.e-6_r8
              ! Write to 6 so all tasks show up on CESM log
              write(6, *) "Negative numberconc", m, tmpa, "#/cm3", k, mm
           end if
        end do
     end do
     if (num_neg > 0) then
        call endrun("dropmixnuc_oslo: "//int2str(num_neg)//" negative number concentration values")
     end if
     ...
     do i = 1, ncol
        ...
        do k=top_lev, pver
           ...
           do l = 1, pcnst
              ...
              if(sumFraction > 1.e-2_r8)then  !Just scale what comes out if componentFraction is larger than 1%
                 do m = 1, ntot_amode
                 ...
                 do m = 1, ntot_amode
                    do lptr = 1, getNumberOfTracersInMode(m)
                       if(getTracerIndex(m,lptr,.FALSE.) == l ) then
                          sumFraction = sumFraction + 1.0_r8
                       endif
                    end do ! tracers in mode
                 end do    ! mode
                 do m=1,ntot_amode
                    componentFractionOK(k,m,l) = 1.0_r8/max(eps, sumFraction)
                 end do !modes
              endif
           end do !tracers
        end do    !levels
        ...
        do m = 1, nmodes ! Number of modes
           ...
           do k= top_lev,pver
              ...
              raercol_cw(k,mm,nsav) = ncldwtr(i,k)*numberConcentration(i,k,m)&
                   /max(eps, sum(numberConcentration(i,k,1:nmodes)))
           enddo
           ...
           if (cldn_tmp-cldo_tmp > 0.01_r8) then
              ...
              do kcomp = 1, nmodes
                 if(hasAerosol(i,k,kcomp)) then
                    ...
                    hygro(m) = max(hygroscopicity(i,k,kcomp), min_hygro)
              ...
              numberOfModes = m
              ! XXgoldyXX: What are these?
              A = 2.25_r8
              B = 1.2_r8
              ACCOM = 1.0_r8

              press=287._r8*cs(i,k)*temp(i,k)
              call CCNSPEC(naermod, DPGI, sigi, modtype, temp(i,k), press,    &
                   numberOfModes, hygro, A, B, SG)
              call PDFACTIV(wbar, naermod, hygro, A, B, ACCOM, SG, wmix,      &
                   temp(i,k), press, NDACT, actfrac, mactfrac, numberOfModes, &
                   SMAX)
              ...
              !!XXgoldyXX: v This section commented out for FORCeS
              ...
              !!XXgoldyXX: ^ This section commented out for FORCeS
